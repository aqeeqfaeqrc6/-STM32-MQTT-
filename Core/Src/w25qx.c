...
void W25Qx_Test(void)
{
    ...
    /*-Step4- 读数据  ************************************************Step4*/	
	if(W25Qx_Read(rData,0x00,256)== W25Qx_OK)
		printf("QSPI Read ok\n");
	else
	...
	printf("\n");
	/*-Step5- 数据对比  ************************************************Step5*/		
	if(memcmp(wData,rData,256) == 0 ) 
	{
			printf("W25Q32FV QuadSPI Test OK\n");
			OLED_ShowStr(0,4,(unsigned char *)"W25Q32  Test OK",2); 
	}
	else
	{
			printf("W25Q32FV QuadSPI Test False\n");
			OLED_ShowStr(0,4,(unsigned char *)"W25Q32  Test Fail",2); 
	}
